...
if sys.version_info.major < 3:
    print("Ups! GeoMapX needs to run with Python 3. It seems you launched it with Python 2. Try using: python3 run.py ... ")
    sys.exit(1)
...
if __name__ == '__main__':
    ...
    log.ODM_INFO('Initializing GeoMapX %s - %s' % (odm_version(), system.now()))
    ...
    if retcode == 0 and not "submodels" in args.project_path:
        ...
        log.ODM_INFO('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
        log.ODM_INFO('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
        log.ODM_INFO('GeoMapX app finished - %s' % system.now())
    else:
        exit(retcode)
